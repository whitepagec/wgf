<? 
	...
	if ($action == "Save")
	{
		try
		{
			$sGST = EncodeHTMLEntities($_POST["tbGST"],1);
			
			$conn = ConnSQL();
			$stmt = $conn->stmt_init();
			
			$query = "UPDATE $sTable SET gst = ?, last_updated_date = now(), last_updated_by = ? WHERE id=?";
			...
			{ 
				$stmt->bind_param("sss", $sGST, $_SESSION["administrator_id"], $id);
				$stmt->execute();
			
				LogEntry($_SESSION["administrator_id"], $_SESSION["administrator_email"], "Updated", "GST", "gst.php", $id);
	...
	else
	{
		try
		{
			...
			else
			{
				...
				{
					$sGST = DecodeHTMLEntities($row["gst"]);
				}
				$stmt->close();
				$conn->close();
			}
...
<head>
    ...
    <script type="text/javascript">
        function SaveClick() 
		{
            try {	
				checkInput('VN','GST');			
				if (document.getElementById("emGST").innerHTML == "")
				{
...
<body>
  ...
  <div style="width:100%;">
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="content">
      ...
      <tr>
        <td></td>
        <td valign="middle">GST: [Please exclude "%"]</td>
        <td>&nbsp;</td>
        <td valign="middle"><input type="text" id="tbGST" name="tbGST" value="<? echo $sGST; ?>" style="width:300px;" maxlength="150" class="textfield" /></td>
		<td valign="middle" align="left"><div id="emGST" class="error_msg" style="width:200px;"></div></td>
      </tr>
      <tr>
        <td colspan="5">&nbsp;</td>
      </tr>
