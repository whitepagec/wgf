		...
		try
		{
			...
			if ($_FILES["tbImage"]["name"] == "")
			{
				...
				{ 
					...
					LogEntry($_SESSION["administrator_id"], $_SESSION["administrator_email"], "Updated", "Product Category", "product_category_update.php", $id);
			...
			else
			{
				if (($_FILES["tbImage"]["type"] == "image/jpg" || $_FILES["tbImage"]["type"] == "image/jpeg" || $_FILES["tbImage"]["type"] == "image/gif" || $_FILES["tbImage"]["type"] == "image/png") && $_FILES["tbImage"]["size"] <= 2000000)
				{
					...
					else
					{
						...
						{
							$query = "UPDATE $sTable SET title = ?, title_chinese = ?, description = ?, image = ?, last_updated_date = now(), last_updated_by = ? WHERE id=?";
							...
							{ 
								$sImage = $id.".".$extension;
								$stmt->bind_param("ssssss", $sTitle, $sTitleChinese, $sDescription, $sImage, $_SESSION["administrator_id"], $id);
								$stmt->execute();
        ...
        function SaveClick() 
		{
            try {	
				...
				if (document.getElementById("emTitle").innerHTML == "")
				{
					...
					document.getElementById("btnViewImage").disabled = true;
...
<body>
  ...
  <div style="width:100%;">
	...
	if ($sImage != "")
	{
	?>&nbsp;&nbsp;<input type="button" id="btnViewImage" name="btnViewImage" onClick="window.open('<? echo $sImageViewPath; ?>product/category/<? echo $sImage; ?>','_blank')" value="View Image" width="80" /><?
	}
	?>
