<? 
	...
	if ($action == "position")
	{
		...
		LogEntry($_SESSION["administrator_id"], $_SESSION["administrator_email"], "Position Changed", "Product Price", "product_price.php", $id);
	...
	else if ($action == "delete")
	{
		try
		{
			...
			else
			{
				...
				$sRedirect = true;
				LogEntry($_SESSION["administrator_id"], $_SESSION["administrator_email"], "Deleted", "Product Price", "product_price.php", $id);
			}
		}
		catch (Exception $e) 
		{
			...
			else
			{
				...
				{	 
					...
					if (DecodeHTMLEntities($row["status"]) == "active")
					{
						...
						LogEntry($_SESSION["administrator_id"], $_SESSION["administrator_email"], "Deactivated", "Product Price", "product_price.php", $id);
					}
					else
					{
						...
						LogEntry($_SESSION["administrator_id"], $_SESSION["administrator_email"], "Activated", "Product Price", "product_price.php", $id);
					}
  ...
  <table cellpadding="0" cellspacing="1" border="0" class="content" bgcolor="#F3F3F3" width="100%">
    <tr>
      <td width="15%" class="gridHeader">
        <a href="javascript:void(0);" onClick="sortTable('Price','Price','<? echo $_SESSION[$sPageName.'PageNos']; ?>');">
        <table cellpadding="0" cellspacing="0" border="0">
          <tr>
            <td>Price</td>
            <td width="10"></td>
            <td><div id="divPriceAsc" style="display:<? if ($_SESSION[$sPageName.'SortField'] == "Price" && $_SESSION[$sPageName.'SortDir'] == "ASC") { echo "block"; } else { echo "none"; } ?>;"><img src="images/icon_arrow_asc.gif" border="0" /></div>
			<div id="divPriceDesc" style="display:<? if ($_SESSION[$sPageName.'SortField'] == "Price" && $_SESSION[$sPageName.'SortDir'] == "DESC") { echo "block"; } else { echo "none"; } ?>;"><img src="images/icon_arrow_desc.gif" border="0" /></div></td>
          </tr>
        </table>
        </a>
      </td>
      <td width="15%" class="gridHeader">
        <a href="javascript:void(0);" onClick="sortTable('PriceGST','PriceGST','<? echo $_SESSION[$sPageName.'PageNos']; ?>');">
        <table cellpadding="0" cellspacing="0" border="0">
          <tr>
            <td>Price with GST</td>
            <td width="10"></td>
            <td><div id="divPriceGSTAsc" style="display:<? if ($_SESSION[$sPageName.'SortField'] == "PriceGST" && $_SESSION[$sPageName.'SortDir'] == "ASC") { echo "block"; } else { echo "none"; } ?>;"><img src="images/icon_arrow_asc.gif" border="0" /></div>
			<div id="divPriceGSTDesc" style="display:<? if ($_SESSION[$sPageName.'SortField'] == "PriceGST" && $_SESSION[$sPageName.'SortDir'] == "DESC") { echo "block"; } else { echo "none"; } ?>;"><img src="images/icon_arrow_desc.gif" border="0" /></div></td>
          </tr>
        </table>
        </a>
      </td>
      ...
      <td width="28%" class="gridHeader">Actions</td>
    </tr>
    <?
	...
	try
	{
		$query = "SELECT gst FROM gst WHERE id = 1";
		...
		else
		{
			...
			if ($row = $result->fetch_assoc())
			{ 
				$sGST = (int)DecodeHTMLEntities($row["gst"]);
			}
		}
	...
	<tr bgcolor="<? echo $sBGColor; ?>" onMouseOver="changeColor(this,'#D0D6DA');" onMouseOut="changeColor(this,'<? echo $sBGColor; ?>')">
      <td class="gridContent">$<? echo DecodeHTMLEntities($row["price"]); ?></td>
      <td class="gridContent">$<? echo number_format(round(((float)DecodeHTMLEntities($row["price"]) * (($sGST/100) + 1)),1),2); ?></td>
