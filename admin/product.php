<? 
	...
	if ($action == "position")
	{
		SwitchPosition($sTable, $_REQUEST["positionid"], $_REQUEST["position"], "AND cat_id = ".$_SESSION[$sPageName.'CatID']);
		LogEntry($_SESSION["administrator_id"], $_SESSION["administrator_email"], "Position Changed", "Product", "product.php", $id);
	...
	else if ($action == "delete")
	{
		try
		{
			...
			else
			{
				...
				if ($row = $result->fetch_assoc()) 
				{
					ShiftPosition($sTable, $row["position"], "AND cat_id = ".$_SESSION[$sPageName.'CatID']);
				}
			}
			...
			else
			{
				...
				$sRedirect = true;
				LogEntry($_SESSION["administrator_id"], $_SESSION["administrator_email"], "Deleted", "Product", "product.php", $id);
			}
		}
		catch (Exception $e) 
		{
			...
			else
			{
				...
				{	 
					...
					if (DecodeHTMLEntities($row["status"]) == "active")
					{
						...
						LogEntry($_SESSION["administrator_id"], $_SESSION["administrator_email"], "Deactivated", "Product", "product.php", $id);
					}
					else
					{
						$sPosID = (string)((int)GetMaxPosition($sTable." WHERE cat_id = ".$_SESSION[$sPageName.'CatID'])+1);
						$query = "UPDATE $sTable SET status = 'active', position = ?, last_updated_date = now(), last_updated_by=? WHERE id=?";
						LogEntry($_SESSION["administrator_id"], $_SESSION["administrator_email"], "Activated", "Product", "product.php", $id);
					}
...
<div style="width:100%;">
  ...
  <table cellpadding="0" cellspacing="1" border="0" class="content" bgcolor="#F3F3F3" width="100%">
    <tr>
      ...
      <td width="10%" class="gridHeader">Price</td>
	...
	try
	{
		$query = "SELECT COUNT(*) as total FROM $sTable WHERE cat_id = ?";
		...
		$query = "SELECT * FROM $sTable WHERE cat_id = ? ORDER BY ".$_SESSION[$sPageName.'SortField']." ".$_SESSION[$sPageName.'SortDir'];
		...
		else
		{
			...
			{ 
				if ($intCount >= $minCount && $intCount < $maxCount)
				{
					...
					$sPriceTotal = DecodeHTMLEntities($row["id"]);
					$query = "SELECT COUNT(*) AS total FROM product_price WHERE pro_id = ?";
					if(!$stmt->prepare($query))
					{
						throw new Exception('SQL ERROR: Failed to prepare statement');
					}
					...
					{
						$stmt->bind_param("s", $sPriceTotal);
						$stmt->execute();

						$resultSub = $stmt->get_result();
						if ($rowSub = $resultSub->fetch_assoc())
						{ 
							$sPriceTotal = DecodeHTMLEntities($rowSub["total"]);
						}
					}
					
					$sImageTotal = DecodeHTMLEntities($row["id"]);
					$query = "SELECT COUNT(*) AS total FROM product_image WHERE pro_id = ?";
					if(!$stmt->prepare($query))
					{
						throw new Exception('SQL ERROR: Failed to prepare statement');
					}
					...
					{
						$stmt->bind_param("s", $sImageTotal);
						$stmt->execute();

						$resultSub = $stmt->get_result();
						if ($rowSub = $resultSub->fetch_assoc())
						{ 
							$sImageTotal = DecodeHTMLEntities($rowSub["total"]);
						}
					}
	?>
	<tr bgcolor="<? echo $sBGColor; ?>" onMouseOver="changeColor(this,'#D0D6DA');" onMouseOut="changeColor(this,'<? echo $sBGColor; ?>')">
      ...
      <td class="gridContent"><?php 
		...
		{
			echo CreateDropDown($sTable." WHERE cat_id = ".$_SESSION[$sPageName.'CatID'],(int)DecodeHTMLEntities($row["position"]),$row["id"]);
		} 
	  ?></td>
      <td class="gridContent"><? echo "<a href='javascript:void(0);' onclick='window.open(\"product_price.php?catID=".DecodeHTMLEntities($row["id"])."\",\"_self\");'>".$sPriceTotal."</a>"; ?></td>
      <td class="gridContent"><? echo "<a href='javascript:void(0);' onclick='window.open(\"product_image.php?catID=".DecodeHTMLEntities($row["id"])."\",\"_self\");'>".$sImageTotal."</a>"; ?></td>
