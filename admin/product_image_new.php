<? 
	...
	if ($action == "Save")
	{
		try
		{
			...
			{
				$query = "INSERT INTO $sTable (pro_id,position,title,image,created_date,created_by,last_updated_date,last_updated_by,status) values (?,?,?,'',now(),?,now(),?,'active')";
				...
				{ 
					$stmt->bind_param("sssss", $_SESSION[$sPrevPageName.'CatID'], $sMaxPosID, $sTitle, $_SESSION["administrator_id"], $_SESSION["administrator_id"]);
					$stmt->execute();
				...
				LogEntry($_SESSION["administrator_id"], $_SESSION["administrator_email"], "Added", "Product Image", "product_image_new.php", $sTempID);
			}
			else
			{
					...
					else
					{
						...
						if(move_uploaded_file($_FILES['tbImage']['tmp_name'], $sImagePath."product/image/".$id.".".$extension)) 
						{
							...
							{ 
								...
								LogEntry($_SESSION["administrator_id"], $_SESSION["administrator_email"], "Added", "Product Image", "product_image_new.php", $id);
...
<head>
    ...
    <script type="text/javascript">
        function SaveClick() 
		{
            try {	
				checkInput('V','Image');		
				if (document.getElementById("emImage").innerHTML == "")
				{
